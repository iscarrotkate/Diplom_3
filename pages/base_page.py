...
class BasePage:
    ...
    def is_element_with_text_present(self, text, parent=None):
        context = parent if isinstance(parent, WebElement) else self.driver

        try:
            element = WebDriverWait(context, 5).until(expected_conditions.visibility_of_element_located((By.XPATH, f".//*[contains(., '{text}')]")))
